...
export const useTradeStore = create<TradeStoreState>()((set, get) => ({
  ...
  getQuoteTokenURL: () => {
    ...
    if (+amountInput) {
      const amount = parseFloat(sendAmount) * Math.pow(10, sendToken.decimals);
      const url = `${quoteUrl}?inputMint=${sendToken.address}&outputMint=${receiveToken.address}&amount=${amount}`;
      return url;
    }
  },
